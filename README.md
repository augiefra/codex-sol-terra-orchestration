...
It uses documented `features.multi_agent` and `[agents]` settings only — no
model catalog patch, compatibility override, or custom agent profile.
...
flowchart TD
    ...
    S --> Q{"Independent and bounded?"}
    Q -->|"No"| S
    Q -->|"Yes"| T["Native Terra High subagent"]
...
High as the default subagent. An explicit spawn setting can still take
precedence, as documented by OpenAI.
