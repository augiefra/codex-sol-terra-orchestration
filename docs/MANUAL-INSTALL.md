...
The fragment sets Sol Ultra as the parent default, enables the native
`multi_agent` and stable opt-in `multi_agent_v2` features, caps child
concurrency at eight, and sets Luna Max as the native leaf default. It adds no
custom agent, catalog override, compatibility patch, or unsupported flag.

Before merging on an older or unfamiliar client, run `codex features list`.
The installed client must list `multi_agent_v2`; update Codex instead of adding
an unknown key if it does not. Do not configure advanced V2 fields unless a
separate, documented requirement calls for them.
...
Also confirm that `codex features list` reports both `multi_agent` and
`multi_agent_v2` as enabled. Resolve all errors, open a fresh task, and restart
Codex only if that fresh task still sees stale settings. Then follow
